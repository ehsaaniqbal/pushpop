...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze head-7 patching under tighter previous-state controls."
    )
...
def main() -> None:
    args = parse_args()
    if args.position <= 0:
        raise ValueError("position must be at least 1 so there is a previous state to inspect")
    ...
    bucketed_pairs = build_pair_buckets(all_pairs, position_rows=position_rows, previous_position=args.position - 1)

    bucket_results: dict[str, object] = {}
    for bucket_name, candidate_pairs in bucketed_pairs.items():
        selected_pairs = candidate_pairs[: args.max_pairs]
        bucket_results[bucket_name] = {
            "candidate_pair_count": len(candidate_pairs),
            "selected_pair_count": len(selected_pairs),
            "previous_state_stats": summarize_previous_state_stats(
                candidate_pairs,
    ...
    output = {
        "analysis": "pp0_head_token_controlled_state_v1",
...
def stack_state_after_position(row: dict[str, object], position_index: int) -> dict[str, object]:
    ...
    return {
        ...
        "slot_1": stack_after[-2] if len(stack_after) > 1 else "EMPTY",
    }
    ...
    *,
    ...
    for pair in candidate_pairs:
        ...
        if source_prev_state["top"] != target_prev_state["top"]:
            buckets["same_prev_token_diff_prev_top"].append(pair)
            continue
        if source_prev_state["depth"] != target_prev_state["depth"]:
            buckets["same_prev_token_same_prev_top_diff_prev_depth"].append(pair)
            continue
        if source_prev_state["slot_1"] != target_prev_state["slot_1"]:
            buckets["same_prev_token_same_prev_top_same_prev_depth_diff_prev_slot1"].append(pair)
    return dict(buckets)


def summarize_previous_state_stats(
    candidate_pairs: list[object],
    *,
    position_rows: list[dict[str, object]],
    previous_position: int,
    ...
    if not candidate_pairs:
        return {
            ...
            "same_prev_slot_1_fraction": 0.0,
            "source_previous_token_counts": {},
        }
    ...
    same_prev_depth = 0
    same_prev_slot_1 = 0
    source_previous_token_counts: Counter[str] = Counter()
    ...
    for pair in candidate_pairs:
        ...
        source_previous_token_counts[source_prev_token] += 1
        ...
        same_prev_slot_1 += int(source_prev_state["slot_1"] == target_prev_state["slot_1"])
    ...
    return {
        ...
        "same_prev_slot_1_fraction": same_prev_slot_1 / pair_count,
        "source_previous_token_counts": dict(sorted(source_previous_token_counts.items())),
    }
